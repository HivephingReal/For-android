...
class EntroapiController extends Controller
{
    ...
    {
        $this->middleware('jwt.auth', ['except' => ['copy_photo_from_api','check_server']]);
//        $this->middleware('block');

        // $this->middleware('type', ['except' => ['other_company_detail','copy_photo_from_api']]);
        $this->middleware('NeedToRegister', ['except' => ['company_register_form_first','check_server', 'copy_photo_from_api', 'company_register_form_second', 'company_register_form_third']]);
...
//                'logo' => 'mimes:jpeg,bmp,png,jpg,gif'
    ...
    public function company_edit(Request $request)
    {
       ...
       public function check_server(){
        ...
        if($_GET['version'] == 1){
            return response()->json(['status'=>true,'title'=>'Allow','link'=>'','message'=>'','positive_text'=>'','negative_text'=>'','success'=>$check_token]);

        }
        else{
            return response()->json(['status'=>false,'title'=>'Need To Download New Version','link'=>'https://play.google.com/store/apps/details?id=com.facebook.katana','message'=>'New features','positive_text'=>'Download','negative_text'=>'Cancle','success'=>$check_token]);
