...
class ConstructprojectController extends Controller
{
    ...
    public function get_confirmed_projects()
    {
...
//        $data = DB::connection('mysql_service')->table('for_repair')->where([['city', '=', $con_com_data->city_id], ['close', 0], ['confirm', '=', 'confirmed']])->whereIn('fr_type',$fr);
        ...
        $get_con = DB::connection('mysql_service')->table('request')->where([['status', '=', 'con'], ['requester_id', '=', JWTAuth::user()->id]])->orderBy('updated_at','desc');
...
//        $user_saw_this=DB::table('user_saw_this_plan')->where([['project_id','=',$data->id],['user_id',JWTAuth::user()->id]])->count();
        ...
        foreach ($get_con->get() as $gc) {
            $d = DB::connection('mysql_service')->table('for_repair')->where([['id','=', $gc->post_id],['delete','=',NULL]])->orWhere([['id','=', $gc->post_id],['delete','=',0]])->whereIn('fr_type', $fr)->first();
...
//            }
            ...
            $get_att1 = DB::connection('mysql_service')->table('attachment')->where([['project_id', '=', $d->id], ['position', '=', 1]]);
            if ($get_att1->count() != 0) {
                $d->file1 = $get_att1->first()->file_name;
            } else {
                $d->file1 = 'empty';
            ...
            //att3
            $get_att3 = DB::connection('mysql_service')->table('attachment')->where([['project_id', '=', $d->id], ['position', '=', 3]]);
            if ($get_att3->count() != 0) {
                $d->file3 = $get_att3->first()->file_name;
            } else {
                $d->file3 = 'empty';
            ...
            //att2
            $get_att2 = DB::connection('mysql_service')->table('attachment')->where([['project_id', '=', $d->id], ['position', '=', 2]]);
            if ($get_att2->count() != 0) {
                $d->file2 = $get_att2->first()->file_name;
            } else {
                $d->file2 = 'empty';
...
//            }
    ...
    public function get_projects()
    {
        ...
        // $data = DB::connection('mysql_service')->table('for_repair')->where([['city', '=', $con_com_data->city_id], ['delete', '=',1], ['confirm', '=', 'confirmed']])->whereIn('fr_type',$fr);
        // under function is temp function
        $data = DB::connection('mysql_service')->table('for_repair')->where([['city', '=', $con_com_data->city_id], ['delete', '=',NULL], ['confirm', '=', 'confirmed']])->orWhere([['city', '=', $con_com_data->city_id], ['delete', '=',0], ['confirm', '=', 'confirmed']])->whereIn('fr_type',$fr)->orderBy('id','desc')->limit(50);
    ...
    public function detail($id)
    {
        ...
        if ($castp == 0) {
            ...
            if ($limit->count() > 0) //if user's have free plan
            {
                ...
                if ($limit->count() != 0) //if user's free plan is not expired
                {
                    //for free plan
